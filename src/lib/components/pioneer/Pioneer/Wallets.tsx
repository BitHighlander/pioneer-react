...
export default function Wallets({ wallets }: { wallets: Wallet[] }) {
    ...
    const currentWallets = wallets.slice((currentPage - 1) * walletsPerPage, currentPage * walletsPerPage);
            ...
            {currentWallets.map((wallet: Wallet, index: number) => (
                <Box key={index}>
                    <HStack spacing={4} alignItems="center">
                        <Avatar src={wallet.icon}></Avatar>
                        <Box>
                            <small>Context: <MiddleEllipsis text={wallet?.context} /></small>
                            <br />
