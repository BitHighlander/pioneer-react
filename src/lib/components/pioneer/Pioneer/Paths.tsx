...
export default function Paths({ paths }: { paths: any[] }) {
    ...
    const handleSelectClick = async (path: Path) => {
        try{
            ...
            let blockchainFromPioneer = await api.GetBlockchain({symbol:path.symbol});
                            ...
                            <Box>
                                {path && (
                                    <>
                                        ...
                                        <div>
                                            <strong>Available Script Types:</strong> {path.available_scripts_types?.join(", ")}
                                        </div>
                                        <div>
                                            <strong>Type:</strong> {path.type}
                                        </div>
                                        <div>
                                            <strong>Address N List:</strong> {path.addressNList?.join(", ")}
                                        </div>
                                        <div>
                                            <strong>Address N List Master:</strong> {path.addressNListMaster?.join(", ")}
                                        </div>
                                        <div>
                                            <strong>Curve:</strong> {path.curve}
                                        </div>
                                        <div>
                                            <strong>Show Display:</strong> {path.showDisplay ? 'true' : 'false'}
                                        </div>
                                    </>
                                )}
                            </Box>
